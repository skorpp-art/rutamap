...
import { FileSpreadsheet, Package, Upload, ChevronLeft, ChevronRight, X, Check } from "lucide-react";
import {
      ...
      <div className="border rounded-xl p-4 bg-slate-50 dark:bg-slate-800/40 space-y-2">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="text-xs text-slate-600 dark:text-slate-300 space-y-1">
            <p className="flex items-center gap-1.5"><Check className="h-3.5 w-3.5 text-emerald-600 shrink-0" /> Columna A: nombre del cliente</p>
            <p className="flex items-center gap-1.5"><Check className="h-3.5 w-3.5 text-emerald-600 shrink-0" /> Columna B: cantidad de paquetes</p>
            <p className="flex items-center gap-1.5"><Check className="h-3.5 w-3.5 text-emerald-600 shrink-0" /> Fecha auto-detectada del nombre del archivo</p>
            ...
            {clientesManuales.map(c => (
              <div key={c.id} className="flex items-center gap-2 text-xs bg-white/70 rounded-lg px-3 py-1.5">
                ...
                <button onClick={() => quitarClienteManual(c.id)}
                  ...
                  title="Quitar"><X className="h-3.5 w-3.5" /></button>
              </div>
            ))}
          </div>
        )}
